...
import BookmarkButton from '../../Bars/NavBar/bookmarkButton';
import {Card, CardHeader, CardBody, CardFooter, Divider, Link, Image} from "@heroui/react";
import { useState, useEffect } from 'react';

const FlightCardUser = ({ flight, passengers, returnDate }) => {
    ...
    const displayPrice = Number(totalPrice).toFixed(2);
    const totalPriceWithPassengers = Number(passengers) * totalPrice;
        ...
        <Card className="max-w-[90%] mx-auto">
            <CardHeader className="flex gap-2 py-2">
                ...
                <div className="w-full flex justify-end">
                    ...
                    <div
                        className="bg-blue-600 hover:bg-blue-700 flex text-white font-semibold rounded-lg transition-colors duration-200"
                    >
                        <BookmarkButton flight={flight} />
                    </div>
                </div>
            </CardHeader>
            <Divider />
            ...
            <CardFooter className="relative flex justify-center items-center py-2">
                <div className="flex items-center justify-center text-center">
                    <button className="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-1.5 px-3 rounded-lg transition-colors duration-200 text-sm">
                        {returnFlight ? 'Select Round Trip' : 'Select Flight'} → ${displayPrice}
                    </button>
                </div>
                {Number(passengers) > 1 && (
                    <div className="text-gray-500 ml-2 justify-end text-xs">
                        * ${totalPriceWithPassengers.toFixed(2)}
                    </div>
                )}
            </CardFooter>
        </Card>
